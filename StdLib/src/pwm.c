...
/** @addtogroup NUC123_PWM_Driver PWM Driver
  @{
*/


/** @addtogroup NUC123_PWM_EXPORTED_FUNCTIONS PWM Exported Functions
  @{
*/
...
{
    (pwm)->TCON = ((pwm)->TCON & ~PWM_TCON_PWM0TEN_Msk | (PWM_TCON_PWM0TEN_Msk << u32ChannelNum));
}
    ...
    for(i = 0; i < PWM_CHANNEL_NUM; i ++)
    {
        if(u32ChannelMask & (1 << i))
        {
            (pwm)->CAPPDMACTL = ((pwm)->CAPPDMACTL & ~(PWM_CAPPDMACTL_CAP0RFORDER_Msk << (i * 8))) | \
                                (((u32RisingFirst << PWM_CAPPDMACTL_CAP0RFORDER_Msk) | \
...
/*@}*/ /* end of group NUC123_PWM_EXPORTED_FUNCTIONS */

/*@}*/ /* end of group NUC123_PWM_Driver */
